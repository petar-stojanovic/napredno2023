package src.kol_1.task_scheduler_trial;
...
class TaskRunner<T extends Task> {
    public <T extends Task > void run(TaskScheduler<T> scheduler, T[] tasks) {
...
//               return Arrays.stream(tasks).sorted(Comparator.comparingInt(Task::getOrder)).collect(Collectors.toList());
               return Arrays.stream(tasks).sorted(Comparator.comparing(x -> x.getOrder())).collect(Collectors.toList());
           }
       };
    }
    ...
    public static <T extends Task> TaskScheduler<T> getFiltered(int order) {
        ...
        return tasks -> Arrays.stream(tasks).filter(it -> it.getOrder() <= order).collect(Collectors.toList());
    }
}