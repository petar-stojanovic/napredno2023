...
class RuleProcessor {
    public static <IN, OUT> void process(List<IN> input, List<Rule<IN, OUT>> rules) {
        ...
        input.stream().forEach(it -> {
            ...
            rules.stream().map(rule -> rule.apply(it))
                              .forEach(rule -> {
                                  if (rule.isPresent()) {
                                      System.out.println("Result: " + rule.get());
                                  } else {
    ...
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            ...
            rules.add(new Rule<>(
                              (it -> it.grades.size() >= 3),
                              (it -> it.grades.stream().mapToDouble(grade -> grade).max().getAsDouble())
            ));
