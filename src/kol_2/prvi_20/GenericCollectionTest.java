...
interface IHasTimestamp {
    ...
    default String getMonthAndDayOfMonth() {
        return String.format("%02d-%02d", getTimestamp().getMonthValue(), getTimestamp().getDayOfMonth());
    }
}
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    void addGenericItem(String category, T element) {
        collection.putIfAbsent(category, new HashSet<>());
//        collection.get(category).add(element);
        collection.computeIfPresent(category, (k, v) -> {
            v.add(element);
            return v;
        });
    }
    ...
    Collection<T> itemsFromCategories(List<String> categories) {
        return collection.keySet().stream()
                      ...
                      .flatMap(category -> collection.get(category).stream())
                      .collect(Collectors.toCollection(() -> new TreeSet<T>(Comparator.reverseOrder())));
    }
