...
class File implements Comparable<File> {
    ...
    public boolean isHiddenFolderAndSizeLessThan(int size) {
        return name.startsWith(".") && this.size < size;
    }

    public String getMonthAndDayCreated() {
        return String.format("%s-%s", timeCreated.getMonth(), timeCreated.getDayOfMonth());
    }
...
class FileSystem {
    ...
    public List<File> findAllHiddenFilesWithSizeLessThen(int size) {
        return files.values().stream()
                      ...
                      .filter(it -> it.isHiddenFolderAndSizeLessThan(size))
                      .collect(Collectors.toList());
    }

    public int totalSizeOfFilesFromFolders(List<Character> folders) {
        return folders.stream().filter(folder -> files.containsKey(folder))
                      .flatMap(it -> files.get(it).stream())
                      .mapToInt(it -> it.size)
                      .sum();
    }
    ...
    public Map<String, Long> sizeByMonthAndDay() {
        return files.values().stream()
                      .flatMap(Collection::stream)
                      .collect(Collectors.groupingBy(
                                    File::getMonthAndDayCreated,
                                    TreeMap::new,
                                    Collectors.summingLong(it -> it.size)
                      ));
    }
}
